...
const manageSubscriptionStatusChange = async (
  ...
  const { data: organizationData, error: noOrganizationError } =
    await supabaseAdmin
      .from("organizations")
      .select("id")
  ...
  if (noOrganizationError)
    throw new Error(`Customer lookup failed: ${noOrganizationError.message}`);

  const { id: uuid } = organizationData;
